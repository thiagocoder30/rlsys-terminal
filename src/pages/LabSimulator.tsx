  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col space-y-6">
      <div className="flex justify-between items-center mt-2 border-b border-slate-800 pb-4">
        ...
        <button onClick={() => { if(simReport) { setSimReport(null); } else { navigate("/"); } }} className="text-[10px] font-black uppercase tracking-widest text-slate-400 border border-slate-700 px-3 py-1.5 rounded bg-slate-800 hover:bg-slate-700">VOLTAR</button>
      </div>
      ...
      {!simReport ? (
        <div className="bg-[#111827] border border-slate-800 p-6 rounded-2xl space-y-5 shadow-xl">
          <div>
            ...
            <input type="number" step="0.01" value={startBankroll} onChange={(e) => setStartBankroll(e.target.value)} className="w-full bg-[#0B101E] text-white font-mono text-xl p-4 rounded-xl border border-slate-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" />
          </div>
          <div>
          ...
          <div className="pt-4 border-t border-slate-800">
            ...
            <OcrButton onNumbersDetected={handleOcrSimulatorUpload} label="ENVIAR PRINT DA ROLETA" isSim={true} />
          </div>
          {loading && <div className="text-center text-xs text-blue-400 animate-pulse font-mono py-2">Processando 10.000 cenários...</div>}
        </div>
      ) : (
        <div className="space-y-4">
          <div className={`p-6 rounded-2xl border ${simReport.verdict === 'GREEN_LIGHT' ? 'bg-emerald-950/20 border-emerald-900/50' : simReport.verdict === 'WARNING' ? 'bg-yellow-950/20 border-yellow-900/50' : 'bg-red-950/20 border-red-900/50'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#0B101E]/50 p-3 rounded-xl border border-slate-800">
                ...
                <span className={`text-lg font-mono font-bold ${simReport.netProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{simReport.netProfit >= 0 ? '+' : ''}R$ {simReport.netProfit.toFixed(2)}</span>
              </div>
              <div className="bg-[#0B101E]/50 p-3 rounded-xl border border-slate-800">
