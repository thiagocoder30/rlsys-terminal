...
function sanitizeDatabaseUrl(url: string | undefined): string | undefined {
  ...
  try {
    ...
    if (protocolMatch) {
      const protocol = protocolMatch[1]; const remainder = protocolMatch[2]; const lastAtIndex = remainder.lastIndexOf("@");
      if (lastAtIndex !== -1) {
        const credentials = remainder.substring(0, lastAtIndex); let hostPart = remainder.substring(lastAtIndex + 1);
        if (hostPart.includes("supabase.co") && !hostPart.includes("sslmode=")) { hostPart += hostPart.includes("?") ? "&sslmode=require" : "?sslmode=require"; }
        const firstColonIndex = credentials.indexOf(":");
        if (firstColonIndex !== -1) {
          const user = credentials.substring(0, firstColonIndex); const pass = credentials.substring(firstColonIndex + 1);
          if (/[@#!:?&]/.test(pass) && !pass.includes("%")) { const encodedPass = encodeURIComponent(pass); return `${protocol}${user}:${encodedPass}@${hostPart}`; }
        }
      }
    }
...
// OTIMIZAÇÃO EXTREMA: CACHE EM MEMÓRIA RAM
...
async function startServer() {
  ...
  const validateUUID = (req: any, res: any, next: any) => {
    const { id } = req.params; const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (id && !uuidRegex.test(id)) return res.status(400).json({ error: "ID de sessão inválido." }); next();
  };
  ...
  app.get("/api/macro", async (req, res) => {
    try {
      ...
      closedSessions.forEach(s => { const profit = s.current_bankroll - s.initial_bankroll; totalProfit += profit; if (profit > 0) winningSessions++; });
  ...
  app.post("/api/sessions", async (req, res) => {
    try {
      ...
      const session = await prisma.session.create({ data: { initial_bankroll, current_bankroll: initial_bankroll, highest_bankroll: initial_bankroll, min_chip: min_chip || 0.50, status: "ACTIVE" } });
      res.json(session);
    } catch (error: any) { res.status(500).json({ error: error.message }); }
  });
    ...
    try {
      ...
      if (lastNumbersInDB.length > 0) {
        for (let i = 0; i < numbers.length; i++) {
          const sub = numbers.slice(0, numbers.length - i); const lastSubInDB = lastNumbersInDB.slice(-sub.length);
          if (sub.length > 0 && JSON.stringify(sub) === JSON.stringify(lastSubInDB)) { newNumbersToInsert = numbers.slice(sub.length); break; }
        }
      }
      ...
      if (!cachedStrategies) cachedStrategies = await prisma.strategy.findMany({ where: { is_active: true } });
      ...
      if (updatedSession) await StrategyOrchestrator.analyzeMarket(recentSpins, cachedStrategies, updatedSession);
  ...
  app.post("/api/sessions/:id/spins", validateUUID, async (req: any, res: any) => {
    try {
      ...
      // OTIMIZAÇÃO: Consultas de Leitura Sequenciais (Evita Timeouts no Supabase Free) e uso de Memória RAM
      if (!cachedStrategies) cachedStrategies = await prisma.strategy.findMany({ where: { is_active: true } });
      const updatedSession = await prisma.session.findUnique({ where: { id } });
      const recentSpins = await prisma.spin.findMany({ where: { session_id: id }, orderBy: { created_at: "desc" }, take: 25 }); // 25 giros é mais que suficiente para o Z-Score calcular
  ...
  app.get("/api/sessions/:id/dashboard", validateUUID, async (req, res) => {
    try {
      ...
      const strategiesStatus = cachedStrategies.map(st => {
  ...
  app.listen(PORT, "0.0.0.0", () => { console.log(`[SECURITY] RL.sys Terminal running on port ${PORT}`); });
}
