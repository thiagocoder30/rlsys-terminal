...
async function startServer() {
  ...
  app.post("/api/signals/:id/action", validateUUID, async (req: any, res: any) => {
    try {
      ...
      if (action !== "CONFIRM" && action !== "REJECT") return res.status(400).json({ error: "Ação inválida." });
      
      const newStatus = action === "CONFIRM" ? "PENDING" : "REJECTED";
      const sig = await prisma.signal.update({ where: { id }, data: { result: newStatus } });
      res.json(sig);
    } catch (error: any) { res.status(500).json({ error: error.message }); }
  });
  ...
  app.post("/api/sessions/:id/spins", validateUUID, async (req: any, res: any) => {
    try {
      ...
      if (!cachedStrategies) cachedStrategies = await prisma.strategy.findMany({ where: { is_active: true } });
      const updatedSession = await prisma.session.findUnique({ where: { id } });
      const recentSpins = await prisma.spin.findMany({ where: { session_id: id }, orderBy: { created_at: "desc" }, take: 25 }); 
